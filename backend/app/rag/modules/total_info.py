...
def aggregate_information(
    ...
    graph_results: dict | None = None,
    user_profile_results: dict | None = None,
    short_term_memory_results: dict | None = None,
) -> dict[str, Any]:
    """Tổng hợp thông tin từ các nguồn khác nhau."""
    ...
    if user_profile_results and user_profile_results.get("success"):
        sources_info.append(
            f"User Profile: {user_profile_results.get('results', {})}"
        )
    ...
    if plan_result:
        ...
        sources_info.append(
            f"Planner Sub Objects: {plan_result.get('sub_objects', [])}"
        )
    ...
    # Gọi LLM để tổng hợp 1 lần duy nhất (không chunk/token truncation).
    aggregated_content = "\n".join(sources_info)
